...
function resolveUrl(baseUrl, relativeUrl) {
  return new URL(relativeUrl, toAbsoluteUrl(baseUrl)).href;
}
...
async function fetchStacObject(url) {
  ...
  return { stacObject: create(data), url: toAbsoluteUrl(url) };
}
...
async function loadStacData() {
  ...
  if (!stacDataPromise) {
    stacDataPromise = (async () => {
      ...
      const { stacObject: rootCatalog, url: rootUrl } = await fetchStacObject(config.stacCatalogUrl);
