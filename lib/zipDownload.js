 ...
 * @param {{mimeType?: string, doc?: Document, autoRevoke?: boolean}} [options]
 *   `doc` is injected for testability; defaults to global document.
 *   `autoRevoke=false` keeps the Blob URL alive for a visible fallback link.
 * @returns {{url: string, filename: string, revoke: Function, blob: Blob}}
 */
 ...
 * @param {{compress?: boolean, doc?: Document, autoRevoke?: boolean}} [options]
 */
export function downloadAsZip(files, archiveName, options = {}) {
  const bytes = buildZip(files, { compress: options.compress });
  return triggerBrowserDownload(bytes, archiveName, {
